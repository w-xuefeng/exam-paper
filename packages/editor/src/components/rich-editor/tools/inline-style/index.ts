...
const ELEMENT_NAME = "editorjs-inline-style";

class EditorJSInlineStyleError extends Error {
  constructor(...args: any[]) {
    super(...args);

    this.name = "EditorJSInlineStyleError";
  }
}

class EditorJSInlineStyleElement extends HTMLElement {}

export class StyleInlineTool implements InlineTool {
  static get isInline() {
    return true;
  }
  ...
  static prepare() {
    ...
    customElements.define(ELEMENT_NAME, EditorJSInlineStyleElement);
  }
  ...
  checkState() {
    ...
    const editorJSStyleElement = this.#api.selection.findParentTag(
      ELEMENT_NAME.toLocaleUpperCase()
    );
    ...
    this.#actions.innerHTML = `
      <div class="editorjs-inline-style-setting" style="margin-bottom: 16px; margin-left: 16px; margin-right: 16px; min-width: 300px;display: flex;flex-direction:column;gap:10px;">
    ...
    if (!deleteButton || !classInput || !idInput || !styleTextarea) {
      throw new EditorJSInlineStyleError();
    }
    ...
    deleteButton.addEventListener("click", () => {
      ...
      const rangeStartNode = clonedNodes[0]!;
      const rangeEndNode = clonedNodes[clonedNodes.length - 1]!;

      range.setStartBefore(rangeStartNode);
      range.setEndAfter(rangeEndNode);
  ...
  surround(range: Range) {
    const editorjsStyleElement = new EditorJSInlineStyleElement();
